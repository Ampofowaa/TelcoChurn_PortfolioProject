...
def _stub_data_content_hash() -> Iterator[None]:
    """run_feature_selection_step stamps data_content_hash via
    features_sha256() with no path override, resolving to the real,
    gitignored datasets/processed/telco_churn_features.parquet — absent on a
    fresh checkout. Every test below trains on synthetic fixtures, never real
    processed data, so stub the hash.
    """
    mp = pytest.MonkeyPatch()
    mp.setattr(
        feature_selection, "features_sha256", lambda path=None: _FAKE_DATA_CONTENT_HASH
    )
    yield
    mp.undo()
