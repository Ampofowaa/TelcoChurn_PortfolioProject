...
- [ ] `docker compose up postgres -d` + `dvc repro` completes end to end on a clean checkout
- [ ] Editing `configs/training/lightgbm.yaml` reruns `train`→`error_analysis` only, not `ingest`/`validate`/`split`/`features`
- [ ] Editing `configs/costs.yaml` reruns `threshold`→`error_analysis` only, leaving `train`/`calibrate` cached
- [ ] Raising `pr_auc_bar` in `configs/model_promotion.yaml` reruns `threshold` and `evaluate`
- [ ] `dvc repro --force` with unchanged deps → `n_trials_run_this_invocation == 0`, same selected trial
- [ ] Adding a 6th passing gate to `checks.py` reruns `validate`+`split` but reports `features` onward cached (content-hash short-circuit)
- [ ] `dvc metrics show` / `dvc plots show` / `dvc metrics diff` render `metrics_summary.json` / `decile_lift.csv` correctly
