...
| Recall at `t*` | guardrail | Blocks the promotion if recall < 0.65, absolute | The same 0.65 minimum every candidate has always had to clear — it doesn't get lowered just because the model currently in production has slipped. A struggling incumbent is something monitoring should flag, not an excuse to accept a weaker replacement. |
...
| Brier | guardrail | Blocks the promotion if the CI on Δ = Brier(challenger) − Brier(champion) lies entirely **above +0.005** | A small dip in this score alone doesn't block promotion — only clear evidence of a genuinely worse result does, not just an inability to prove things are equal or better. A model that ranks customers meaningfully better is still the better choice, even if this score comes out very slightly worse. |
...
| `feature_space.txt` / `feature_columns.txt` | `train.py` | Full feature space vs. the subset that survived selection into the model |
| `preprocessing.pkl` | `train.py` | Fitted `ColumnTransformer` |
| `training_manifest.json` | `train.py` | Git SHA, DVC hash, hyperparameters, tuning summary |
