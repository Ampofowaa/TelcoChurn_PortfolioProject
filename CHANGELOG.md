...
- **`src/telco_churn/models/evaluate.py`** — `_compute_core_test_metrics` now emits a `logger.warning("cost_scenario_spread_too_narrow", ...)` when `sealed_test_business_impact`'s `parameter_spread_dominates_sampling` is `False`, mirroring `threshold.py`'s `threshold_argmax_disagreement` pattern. Previously this diagnostic was only visible by hand-reading the JSON artifact, which Phase 10's unattended monthly retrain never does.
